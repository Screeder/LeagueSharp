...
namespace SAssemblies.Trackers
{
    class Tracker
    {
        ...
        public static Menu.MenuItemSettings SetupMenu(LeagueSharp.Common.Menu menu, bool useExisitingMenu = false)
        {
            ...
            if (!useExisitingMenu)
            {
                Trackers.Menu = Menu.GetSubMenu(menu, "SAssembliesTracker") ?? menu.AddSubMenu(new LeagueSharp.Common.Menu(Language.GetString("TRACKERS_TRACKER_MAIN"), "SAssembliesTracker"));
            }
            else
            {
                Trackers.Menu = menu;
