    ...
    class Altar
    {
...
#else
            return AltarTimer.GetActive();
#endif
        }

        public static Menu.MenuItemSettings SetupMenu(LeagueSharp.Common.Menu menu)
        {
            var newMenu = Menu.GetSubMenu(menu, "SAssembliesTimersAltar");
            if (newMenu == null)
            {
                AltarTimer.Menu = menu.AddSubMenu(new LeagueSharp.Common.Menu(Language.GetString("TIMERS_ALTAR_MAIN"), "SAssembliesTimersAltar"));
                AltarTimer.CreateActiveMenuItem("SAssembliesTimersAltarActive", () => new Altar());
            }
            return AltarTimer;
        }
        ...
        private void Game_OnGameUpdate(EventArgs args)
        {
            ...
            foreach (AltarObject altar in Altars)
            {
                if (altar.Obj.IsValid)
                {
                    ...
                    {
                        if (buff.Name.Equals("treelinelanternlock"))
                        {
                            hasBuff = true;
                            break;
                        }
            ...
            if (altarDestroyed != null)
            {
                altarDestroyed.TextMap.Dispose();
                altarDestroyed.TextMap.Remove();
                altarDestroyed.TextMinimap.Dispose();
                altarDestroyed.TextMinimap.Remove();
                Altars.Remove(altarDestroyed);
            }
        ...
        public void Init()
        {
            foreach (Obj_AI_Minion objectType in ObjectManager.Get<Obj_AI_Minion>())
            {
                ...
                {
                    bool hasBuff = false;
                    foreach (BuffInstance buff in objectType.Buffs)
                    {
                        if (buff.Name.Equals("treelinelanternlock"))
                        {
                            hasBuff = true;
                    ...
                    if (objectType.Name.Contains("_L"))
                        Altars.Add(new AltarObject("Left Altar", objectType, hasBuff));
                    else
                        Altars.Add(new AltarObject("Right Altar", objectType, hasBuff));
                }
            }
        }
        ...
        public class AltarObject
        {
            ...
            public AltarObject(String name, Obj_AI_Minion obj, bool locked = false)
            {
                Name = name;
                Obj = obj;
                SpawnTime = 185;
